...
for i in range(len(t_A)):
    if i < x_A:
        filter_A.append(sum(data1_A[:i+1])/(i+1))
    else: 
        filter_A.append(sum(data1_A[i-x_A+1:i+1])/x_A)

x_B = 200
filter_B = []
for i in range(len(t_B)):
    if i < x_B:
        filter_B.append(sum(data1_B[:i+1])/(i+1))
    else: 
        filter_B.append(sum(data1_B[i-x_B+1:i+1])/x_B)

x_C = 100
filter_C = []
for i in range(len(t_C)):
    if i < x_C:
        filter_C.append(sum(data1_C[:i+1])/(i+1))
    else: 
        filter_C.append(sum(data1_C[i-x_C+1:i+1])/x_C)

x_D = 50
filter_D = []
for i in range(len(t_D)):
    if i < x_D:
        filter_D.append(sum(data1_D[:i+1])/(i+1))
    else: 
        filter_D.append(sum(data1_D[i-x_D+1:i+1])/x_D)
...
fig_A.suptitle('Signal A, Moving Average with %i points' %x_A)
...
fig_B.suptitle('Signal B, Moving Average with %i points' %x_B)
...
fig_C.suptitle('Signal C, Moving Average with %i points' %x_C)
...
fig_D.suptitle('Signal D, Moving Average with %i points' %x_D)
